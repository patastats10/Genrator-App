    ...
    for outcome, marker in markers.items():
        if outcome == "Goal":
            ...
            marker_handles.append(
                pitch.scatter([], [], hexcolor='w', pentcolor='k', linestyle=linestyle_, marker=marker, s=300,  ax=axs_Shots)
            )
        else:
            linestyle_ = linestyles.get(outcome, '-')
            color = colors.get(outcome, 'none')
            edgecolor = edgecolors.get(outcome, 'none')
            marker_handles.append(
                pitch.scatter([], [], hexcolor=color, linestyle=linestyle_, marker=marker, s=300, pentcolor=edgecolor, ax=axs_Shots)
            )
